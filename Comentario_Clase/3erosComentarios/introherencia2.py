class A1: #Se hace uso de la palabra reservada class para crear una clase, el nombre de esya clase sera A1 y finalemente se cierra con dos puntos.
...
class A2: #Se crea una segunda clase usando la palabra reserva class y se le asigna su nombre correspondiente el cual en este caso sera A2.
...
class B(A2,A1): #Se crea una nueva clase llamada B la cual en su lista de parametros tiene dos clases que son A2 y A1 lo que quiere decir que la clase B es una clase hija o subclase, y sus clase padre son la clase A1 y la clase A2, esto tambien quiere decir que la clases B heredara todos los metodos o propiedades de dichas clases haciendolos 'propios'.
    def __init__(self):#En esta parte se crea o instancia el constructor de la clase B el cual en su lista de parametros solo contiene el parametro obligatorio self que se usa para indicar que el constructor estara dentro de la clase B.
        pass #Aqui se usa la palabra reserva pass que indicara que el constructor no tendra propiedades o atributos, esto quiere decir que sera un metodo o un constructor vacio.
...
# cad="sena" #Se crea una variable la cual se llama cad y que contiene una cadena de texto, la cual es "sena".
# lista=[1,2,3] #Se crea una lista que es un dato estructurado al igual que la cadena, y contendra 1 en la posicion 0, 2 en la posicion 1 y 3 en la posicion 2.
# print(cad.__str__()) #Se imprime la cadena, pero se hace el llamado del metodo __str__ el cual msotrara por pantalla las cordenadas de cad en el codigo o cuando se creo el objeto cad.
# print(lista.__str__()) #Se imprime la lsita, pero se hace el llamado del metodo __str__ el cual msotrara por pantalla las cordenadas de la lista con su respectivo contenido en el codigo o cuando se creo el objeto lista.